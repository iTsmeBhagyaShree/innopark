...
const Messages = () => {
  ...
  const [activeTab, setActiveTab] = useState('employees') // 'clients', 'employees', or 'admins'
  ...
  useEffect(() => {
    const loadData = async () => {
      if (selectedConversation) {
        // First fetch and mark as read
        await fetchConversationMessages(selectedConversation.other_user_id)
        // THEN fetch updated conversation list (unread counts will be 0)
    ...
    if (selectedConversation) {
      interval = setInterval(async () => {
        await fetchConversationMessages(selectedConversation.other_user_id)
        // Also refresh conversations to clear badges if messages were marked as read
        fetchConversations()
      }, 5000)
    }
    return () => {
  ...
  const fetchConversationMessages = async (otherUserId) => {
    try {
      ...
      if (response.data.success) {
        ...
        // Mark local list as read for this user immediately
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || t('messages.send_failed'))
    } finally {
      setSending(false)
    }
  }
  ...
  const handleSelectUser = (user) => {
    ...
    // Optimistically update local state to remove the notification badge immediately
    setConversations(prev => prev.map(c => 
    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-140px)]">
        <div className="text-center">
          ...
          <p className="mt-4 text-secondary-text">{t('messages.loading')}</p>
        </div>
      </div>
    )
  }
  ...
  if (userRole === 'SUPERADMIN') {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-primary-text">{t('messages.title')}</h1>
          <p className="text-secondary-text mt-1">{t('messages.subtitle')}</p>
        </div>
        <Card className="p-12 text-center">
          <IoPerson size={64} className="mx-auto mb-4 text-gray-300" />
          <h3 className="text-xl font-semibold text-primary-text mb-2">{t('messages.not_available')}</h3>
          <p className="text-secondary-text">
            {t('messages.no_access')}
            <br />
            {t('messages.role_restriction')}
          </p>
        </Card>
      </div>
    )
    ...
    <div className="space-y-4 h-full flex flex-col">
      ...
      <Card className="overflow-hidden flex-1 shadow-2xl rounded-2xl border border-gray-200" style={{ height: 'calc(100vh - 180px)' }}>
        <div className="flex h-full relative">
          ...
          {/* User List Sidebar */}
          <div className={`${selectedConversation ? 'hidden md:flex' : 'flex'} w-full md:w-96 border-r border-gray-200 flex-col bg-white overflow-hidden`}>
            {/* Tabs (Only for Admin) */}
            {showTabs && (
              <div className="flex border-b border-gray-200 bg-gray-50/50">
                <button
                  onClick={() => setActiveTab('clients')}
                  className={`flex-1 py-4 text-[10px] font-bold border-b-2 transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === 'clients'
                      ? 'border-primary-accent text-primary-accent bg-white shadow-sm'
                      : 'border-transparent text-gray-500 hover:bg-gray-100'
                    }`}
                >
                  <IoPerson size={14} />
                  {t('messages.tabs.clients')}
                </button>
                <button
                  onClick={() => setActiveTab('employees')}
                  className={`flex-1 py-4 text-[10px] font-bold border-b-2 transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === 'employees'
                      ? 'border-primary-accent text-primary-accent bg-white shadow-sm'
                      : 'border-transparent text-gray-500 hover:bg-gray-100'
                    }`}
                >
                  <IoPeople size={14} />
                  {t('messages.tabs.employees')}
                </button>
                <button
                  onClick={() => setActiveTab('admins')}
                  className={`flex-1 py-4 text-[10px] font-bold border-b-2 transition-all duration-200 flex items-center justify-center gap-1.5 ${activeTab === 'admins'
                      ? 'border-primary-accent text-primary-accent bg-white shadow-sm'
                      : 'border-transparent text-gray-500 hover:bg-gray-100'
                    }`}
                >
                  <IoBriefcase size={14} />
                  {t('messages.tabs.admins')}
                </button>
              </div>
            )}
            ...
            <div className="p-4 border-b border-gray-100">
              <div className="relative group">
                <IoSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-primary-accent transition-colors" size={18} />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder={t('messages.search_placeholder').replace('{{tab}}', t(`messages.tabs.${activeTab}`).toLowerCase())}
                  className="w-full pl-11 pr-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:ring-2 focus:ring-primary-accent/20 focus:bg-white outline-none transition-all"
                />
              </div>
            </div>
              ...
              {displayUsers.length === 0 ? (
                <div className="p-12 text-center text-gray-400">
                  <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-gray-100">
                    <IoPerson size={32} className="text-gray-200" />
                  </div>
                ...
                displayUsers.map((conv) => (
                  <div
                    ...
                    className={`p-4 cursor-pointer hover:bg-gray-50 transition-all duration-200 group relative ${selectedConversation?.other_user_id === conv.other_user_id ? 'bg-primary-accent/5 border-l-4 border-l-primary-accent' : 'border-l-4 border-l-transparent'}`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-0.5">
                          <h4 className="text-sm font-bold text-gray-800 truncate group-hover:text-primary-accent transition-colors">
                            {conv.other_user_name}
                          </h4>
                          <div className="flex flex-col items-end">
                            {conv.last_message_time && (
                              <span className="text-[10px] text-gray-400 font-bold uppercase transition-colors group-hover:text-gray-500">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs text-gray-500 truncate italic max-w-[160px]">
                            {conv.last_message || conv.other_user_email}
                          </p>
                          <div className="flex items-center gap-1.5">
                            {(conv.unread_count > 0 && String(conv.other_user_id) !== String(selectedConversation?.other_user_id)) && (
                            ...
                            <Badge variant="default" className="text-[9px] px-1.5 py-0 uppercase tracking-tighter opacity-70">
            ...
            {selectedConversation ? (
              <>
                      ...
                      {showChatMenu && (
                        <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                          <button className="w-full px-4 py-2.5 text-left text-sm font-medium hover:bg-gray-50 text-gray-700 flex items-center gap-3">
                            <IoArchive size={18} className="text-gray-400" /> {t('messages.menu.archive')}
                          </button>
                          <button className="w-full px-4 py-2.5 text-left text-sm font-medium hover:bg-gray-50 text-gray-700 flex items-center gap-3">
                            <IoVolumeOff size={18} className="text-gray-400" /> {t('messages.menu.mute')}
                          </button>
                          <button className="w-full px-4 py-2.5 text-left text-sm font-medium hover:bg-gray-50 text-gray-700 flex items-center gap-3">
                            <IoFlag size={18} className="text-yellow-500" /> {t('messages.menu.important')}
                          </button>
                          ...
                          <button className="w-full px-4 py-2.5 text-left text-sm font-medium text-red-600 hover:bg-red-50 flex items-center gap-3">
                            <IoTrash size={18} /> {t('messages.menu.delete')}
                          </button>
                        </div>
                      )}
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-pattern custom-scrollbar">
                  {messages.length > 0 ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                      ...
                      <h4 className="text-lg font-bold text-gray-800">{t('messages.no_messages_yet')}</h4>
                      <p className="text-gray-400 text-sm mt-1 max-w-[200px]">{(t('messages.start_conversation') || '').replace('{{name}}', selectedConversation.other_user_name)}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 md:p-6 bg-white border-t border-gray-100">
                  <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-center gap-3 bg-gray-50 p-1.5 rounded-2xl border border-gray-200 focus-within:ring-2 focus-within:ring-primary-accent/20 focus-within:bg-white transition-all shadow-sm">
                    <input
                      type="text"
                      value={newMessageText}
                      onChange={(e) => setNewMessageText(e.target.value)}
                      placeholder={t('messages.input_placeholder').replace('{{name}}', selectedConversation.other_user_name)}
                    ...
                    >
                      <IoSend size={20} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200" />
                    </button>
                  </form>
                </div>
              </>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-12 animate-in fade-in duration-700">
                <div className="relative mb-8">
                  <div className="w-32 h-32 rounded-full bg-gradient-to-br from-gray-50 to-gray-200 flex items-center justify-center shadow-inner">
                    <IoPerson size={64} className="text-gray-200" />
                  </div>
                ...
                <h3 className="text-2xl font-black text-gray-800 tracking-tight">{t('messages.select_chat')}</h3>
                <p className="text-gray-500 mt-2 max-w-xs font-medium italic">
